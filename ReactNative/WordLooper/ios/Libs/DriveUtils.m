...
@property (nonatomic, strong) id gAuthentication;
@end
...
-(void) runSigninWithPresentController:(UIViewController*)presentController thenHandler:(void (^)(void))handler {
  
  NSURL *successURL = [NSURL URLWithString:@"com.googleusercontent.apps.665124887696-ed9m78r2f786dpubqgsuoul728hac9v8:/oauthredirect"];
  ...
  appDelegate.currentAuthorizationFlow = [OIDAuthState authStateByPresentingAuthorizationRequest:request
                                 presentingViewController:presentController
                                                 callback:^(OIDAuthState * _Nullable authState, NSError * _Nullable error) {
                                                   ...
                                                   if (authState) {
                                                     ...
                                                     strongSelf.driveService.authorizer = authorization;
                                                     NSLog(@"Got authorization tokens. Access token: %@",
                                                           authState.lastTokenResponse.accessToken);
                                                     [GTMAppAuthFetcherAuthorization saveAuthorization:authorization toKeychainForName:kGTMAppAuthKeychainItemName];
                                                     ...
                                                     strongSelf.driveService.authorizer = nil;
                                                   }
                                                 }];
 
}

- (GTMOAuth2ViewControllerTouch*)runSigninThenHandler:(void (^)(void))handler {
  
  // performs authentication request
  __weak __typeof(self) weakSelf = self;
  self.gAuthentication = [[GTMOAuth2ViewControllerTouch alloc] initWithScope:kGTLRAuthScopeDrive
                                                           ...
                                                           completionHandler:^(GTMOAuth2ViewControllerTouch *viewController, GTMOAuth2Authentication *auth, NSError *error) {
                                                             ...
                                                             if(!error && auth) {
                                                               strongSelf.driveService.authorizer = auth;
